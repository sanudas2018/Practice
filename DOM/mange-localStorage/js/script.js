...
const setProductInLocalStorage = (name, quantity) => {
    ...
    // console.log(parseInt(products[name]))
...
const display = () => {
    ...
    for (const product in products) {
        ...
        div.innerHTML = `<div class="shadow-sm p-3 mb-2 bg-body rounded d-flex align-content-center justify-content-between">
            <span class="fs-4">${name}</span>
            <span>Quantity:<small class="fw-bold">
                ${quantity}
            </small></span>
            <span class=" text-danger" onclick="removeItem()">X</span>
        </div>`;
